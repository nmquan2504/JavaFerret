...
BOOL Init(BOOL bHasUI) {
	...
	if (InitWindow(hInstance, bHasUI)) {
...
HWND GetJavaFerretWindow()
{
	return theDialogWindow;
}

char szAppName[] = "FERRETWINDOW";

char* GetAppLocation(char* szAppLocation)
{
	::GetModuleFileName(hInstance, &szAppLocation[0], MAX_PATH);
	for (int i = strlen(szAppLocation) - 1; i >= 0; i--) 
	{
		if (szAppLocation[i] == '\\') 
		{
			szAppLocation[i + 1] = '\0';
...
*
*/
BOOL InitWindow(HANDLE hInstance, BOOL bHasUI) {
	...
	if (bHasUI==TRUE && !theDialogWindow)
		return (FALSE);

	char szFerretLog[MAX_PATH], szFerretDebug[MAX_PATH];
	GetAppLocation(szFerretLog);
	GetAppLocation(szFerretDebug);
	strcat(szFerretLog, JAVA_FERRET_LOG);
	strcat(szFerretDebug, JAVA_FERRET_DEBUG);

	if (logfile == null) {
		logfile = fopen(szFerretLog, "w"); // overwrite existing log file
		logString(logfile, "Starting JavaFerret.exe %s\n", getTimeAndDate());
	}

	if (debugfile == null) {
		debugfile = fopen(szFerretDebug, "w"); // overwrite existing log file
		logString(debugfile, "Debugging JavaFerret.exe %s\n", getTimeAndDate());
	}
...
void echoMouseObject() {
	...
	logString(debugfile, "Begin echoMouseObject()");
	//

	BOOL bIsJavaWindow = IsJavaWindow(hwnd);
	logString(debugfile, "IsJavaWindow(%X)=%d [%d]", hwnd, bIsJavaWindow, ::GetTickCount() - dw);
	dw = ::GetTickCount();
	...
	if (GetAccessibleContextFromHWND(hwnd, &vmID, &acParent)) {
		...
		// [QMN]
		logString(debugfile, "GetAccessibleContextFromHWND [%d]", ::GetTickCount() - dw);
		dw = ::GetTickCount();
		//
		...
		if (GetAccessibleContextAt(vmID, acParent, (jint)p.x, (jint)p.y, &ac)) {
			...
			// [QMN]
			logString(debugfile, "GetAccessibleContextAt [%d]", ::GetTickCount() - dw);
			dw = ::GetTickCount();
			//
	...
	logString(debugfile, "End echoMouseObject() [%d]", ::GetTickCount() - dw);
	logString(debugfile, " ");
	dw = ::GetTickCount();
}
